...
function personal(){
   ...
   $sql= "UPDATE sturegister SET fname='$fname',lname='$lname' WHERE email='$email'";
...
function pro(){
    ...
    $_SESSION['qua']=$qua;
    $sql= "UPDATE sturegister SET skills='$skill',qualification='$qua' WHERE email='$email'";
    if (mysqli_query($con, $sql)) {
        echo "<script>alert('updated successfully')</script>"; 
...
function name()
{
    ...
    $name=$_POST['name'];
    $_SESSION['name']=$name;
    $sql= "UPDATE sturegister SET fullname='$name' WHERE email='$email'";
    if (mysqli_query($con, $sql)) {
        echo "<script>alert('updated successfully')</script>"; 
...
function about()
{
    ...
    $sql= "UPDATE sturegister SET about='$about' WHERE email='$email'";
    if (mysqli_query($con, $sql)) {
        echo "<script>alert('updated successfully')</script>"; 
...
function pass()
{
    ...
    $sql = "SELECT password FROM sturegister WHERE email='$email'";
...
if (mysqli_num_rows($result) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
    if($current===$row["password"])
    {
        if($newpass===$cnewpass)
        {
            $sqll= "UPDATE sturegister SET password='$newpass' WHERE email='$email'";
...
function upload(){
  require 'dbconnection.php';
    ...
    $_SESSION['photo']=$file;
    $sql= "UPDATE sturegister SET image='$file' WHERE email='$email'";
    if (mysqli_query($con, $sql)) {
        echo "<script>alert('updated successfully')</script>"; 
